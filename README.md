...
- `input/HISTO-*.txt|*_histo.log` — histórico da DRB, obrigatório na validação automática e opcional na execução forçada
...
- A publicação oficial usa GitHub Actions: o workflow de Pages sobe o diretório `public/` como artefato e faz o deploy para a URL acima.
...
- Falhas de identificação do plano ou do evento `[Fire]` interrompem a validação automática; o sistema não usa a data/hora atual como substituição automática.
...
- Se o horário do `[Fire]` não puder ser identificado, a tela solicita o horário local do desmonte. Quando o HISTO não for anexado, o botão **Forçar execução** exige esse horário informado pelo usuário; a data usada é a data local do navegador no momento da execução. Se houver HISTO, mas nenhum horário `[Fire]` legível, a execução forçada sem preenchimento mantém o fallback explícito `12:00:00` local. Horários manuais e esse fallback não sofrem nova conversão de fuso.
